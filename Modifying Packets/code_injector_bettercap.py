...
def process_packet(packet):
    ...
    if scapy_packet.haslayer(scapy.Raw):
        ...
        if scapy_packet[scapy.TCP].dport == 8080: # Bettercap uses port 8080 for HTTP
            print("[+] HTTP Request")
            load = re.sub("Accept-Encoding:.*?\\r\\n", "", load) # Remove the Accept-Encoding header
            load = load.replace("HTTP/1.1", "HTTP/1.0") # Change the HTTP version to 1.0
        
        elif scapy_packet[scapy.TCP].sport == 8080:  # Check if the packet is a response
            print("[+] HTTP Response")
